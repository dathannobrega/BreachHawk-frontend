...
export function SidebarNav({ onCollapseChange }: SidebarNavProps) {
  ...
  const NavContent = () => (
      <div className="flex h-full flex-col bg-white">
        ...
        <div className={cn(
          ...
          {/* Collapse buttons */}
          <Button variant="ghost" size="sm" onClick={handleToggleCollapse} className="hidden lg:flex h-8 w-8 p-0 hover:bg-slate-100">
            {isCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
          </Button>
          <Button variant="ghost" size="sm" onClick={() => setIsMobileOpen(false)} className="lg:hidden h-8 w-8 p-0 hover:bg-slate-100">
            <X className="h-4 w-4" />
          </Button>
        </div>
          ...
          {filteredItems.map(item => {
            ...
            return (
                <Link key={item.href} href={item.href} className={cn(
                    ...
                    isActive ? "bg-blue-100 text-blue-900 border border-blue-200" : "text-slate-700 hover:bg-slate-100 hover:text-slate-900",
                  ...
                  {!isCollapsed && (
                      <>
                        ...
                        {item.badge && <Badge variant="secondary" className="ml-auto bg-red-100 text-red-800">{item.badge}</Badge>}
                      </>
                  )}
                </Link>
            )
          ...
          { !isCollapsed ? (
              <div className="flex items-center space-x-3">
                <Avatar className="h-10 w-10">
                  { user?.profile_image ? (
                      <AvatarImage src={`${apiUrl}${user.profile_image}`} alt={getUserDisplayName()} className="object-cover" />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Badge variant="secondary" className={cn("text-xs mt-1", getRoleColor(user?.role || "user"))}>{getRoleLabel(user?.role || "user")}</Badge>
                </div>
              </div>
          ) : (
              <div className="flex justify-center">
                <Avatar className="h-10 w-10">
                  <AvatarImage src={user?.profile_image ? `${apiUrl}${user.profile_image}` : "/placeholder.svg"} alt={getUserDisplayName()} className="object-cover" />
                  <AvatarFallback className="bg-blue-100 text-blue-700 font-medium">{getUserInitials()}</AvatarFallback>
                </Avatar>
              </div>
          ) }
  ...
  return (
      <>
        {/* Mobile Menu Button */}
        <Button variant="ghost" size="sm" onClick={() => setIsMobileOpen(true)} className="lg:hidden fixed top-4 left-4 z-50 h-10 w-10 p-0 bg-white shadow-md border border-slate-200">
